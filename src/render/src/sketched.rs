...
use bevy::{
    ...
    render::{
        ...
        render_resource::{
            ...
            ShaderType, SpecializedMeshPipelineError, TextureFormat, TextureViewDimension, TextureViewDescriptor,
        },
    },
    utils::HashMap,
};
    ...
    for (e_material, h_std) in material_query.iter() {
        ...
        let h_sketched = match standard_materials.remove(h_std) {
            Some(mut std_material) => {
                // the textures needs to be forced to D2Array
                // in order to prevent wgpu errors
                std_material.base_color_texture = std_material.base_color_texture.map(|tex_handle| {
                    ...
                    // redefine new texture
                    return textures.add(new_tex);
                });
