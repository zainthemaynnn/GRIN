...
macro_rules! bt_internal {
    ( @root $($block:tt)* ) => {
        {
            let mut tree = crate::bt::tree::BehaviorTree::default();
            let root = tree.push_node(crate::bt::tree::Node::default());
    ...
    ( @composite $tree:expr, $parent:expr, $kind:expr, $($block:tt)+ ) => {
        let composite = $tree.push_node(
            crate::bt::tree::Node::Composite(
                crate::bt::tree::Composite::new($kind),
            ),
        );
    ...
    ( @decorator $tree:expr, $parent:expr, $kind:expr, $($block:tt)+ ) => {
        let decorator = $tree.push_node(
            crate::bt::tree::Node::Decorator(
                crate::bt::tree::Decorator::new($kind)
            ),
        );
    ...
    ( @leaf $tree:expr, $parent:expr, $task:expr ) => {
        let leaf = $tree.push_node(
            crate::bt::tree::Node::Leaf(
                crate::bt::tree::Leaf::new($task)
            ),
        );
